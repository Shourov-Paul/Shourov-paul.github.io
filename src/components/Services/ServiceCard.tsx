...
const ServiceCard: React.FC<ServiceCardTypes> = ({ title, shortDescription, icon }) => {
  return (
    <div className="bg-secondary border-border flex flex-col items-center rounded-[14px] border p-5 transition-all duration-300 hover:-translate-y-1 hover:border-accent hover:shadow-lg hover:shadow-accent/5">
      <Image src={icon} alt={title} className="my-1 size-14" />
      <h3 className="text-accent mt-2 mb-5 text-center text-base font-semibold">{title}</h3>
